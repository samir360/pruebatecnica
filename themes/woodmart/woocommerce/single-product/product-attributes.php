	...
	<?php foreach ( $product_attributes as $product_attribute_key => $product_attribute ) : ?>
		...
		<tr class="woocommerce-product-attributes-item woocommerce-product-attributes-item--<?php echo esc_attr( $product_attribute_key ); ?>">
			...
			<td class="woocommerce-product-attributes-item__value">
				...
				$last_key                = array_key_last( $exploded_terms );
				$product_attribute_value = '';

				foreach ( $exploded_terms as $key => $exploded_term ) :
					$term_instance = get_term_by( 'name', $exploded_term, str_replace( 'attribute_', '', $product_attribute_key ) );
					$tooltip       = '';

					if ( $term_instance && ! is_wp_error( $term_instance ) ) {
						$tooltip = get_term_meta( $term_instance->term_id, 'pa_term_hint', true );
					}

					ob_start();
					?>
						<span class="wd-attr-term">
							<?php echo wp_kses_post( $exploded_term ); ?>
					...
					$product_attribute_value .= ob_get_clean();
				endforeach;
				?>
				<p>
					<?php echo wp_kses_post( $product_attribute_value ); ?>
				</p>
			</td>
		</tr>
	<?php endforeach; ?>
